...
// Proporsi aktual dari data historis
const TRANSFER_PCT = 0.67
const CEK_SALDO_PCT = 0.33
const TRANSFER_FEE = 2500
const CEK_SALDO_FEE = 1800
const AVG_FEE_PER_TRX = (TRANSFER_PCT * TRANSFER_FEE) + (CEK_SALDO_PCT * CEK_SALDO_FEE) // ~2269
...
function DerivedCard({ label, value, sub }: { label: string, value: string, sub?: string }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: '11px', color: '#9ca3af', fontWeight: '600', marginBottom: '6px', letterSpacing: '0.05em' }}>
        {label}
      </div>
...
export default function TargetSimplePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Derive semua nilai dari monthly_fee
      ...
      const dailyActiveAgents = Math.round(dailyTrx / 6.5) // ~6.5 TRX/agen aktif/hari baseline
  ...
  const projectedFee = monthStats && monthStats.days_elapsed > 0
    ? Math.round(monthStats.total_fee / monthStats.days_elapsed * monthStats.days_in_month)
    : null
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#6b7280', marginTop: '4px' }}>
            Set target fee bulanan — AMARIS hitung sisanya otomatis.
          </p>
        </div>
        ...
        <div style={{
          ...
          {isNew && (
            <span style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '99px', backgroundColor: '#fef9c3', color: '#ca8a04', fontWeight: '600' }}>
              Belum ada target
            </span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    setMonthlyFee(parseRp(e.target.value))
                  }}
                  onBlur={e => {
                    e.target.style.borderColor = '#e5e7eb'
                    if (monthlyFee) setRawFee(monthlyFee.toLocaleString('id-ID'))
                  }}
                  onFocus={e => e.target.style.borderColor = '#0344D8'}
                  placeholder="contoh: 900.000.000"
                  style={{
                    flex: 1, padding: '12px 16px', borderRadius: '8px',
                    border: '1px solid #e5e7eb', fontSize: '18px', fontWeight: '700',
              ...
              <div style={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: '700', color: '#374151', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: '#9ca3af', fontWeight: '400' }}>
                    berdasarkan rata-rata fee/TRX = {formatRp(Math.round(avgFeePerTrx))}
                  </span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <DerivedCard
                    label="TARGET TRX BULANAN"
                    value={targetMonthlyTrx.toLocaleString('id')}
                    sub={`${targetDailyTrx?.toLocaleString('id')} TRX/hari`}
                  />
                  <DerivedCard
                    label="TARGET FEE HARIAN"
                    value={formatRp(targetDailyFee!)}
                    sub={`dari ${daysInMonth} hari kerja`}
                  />
                  <DerivedCard
                    label="TARGET AGEN AKTIF/HARI"
                    value={targetDailyAgents?.toLocaleString('id') ?? '—'}
                    sub="estimasi baseline 6.5 TRX/agen"
                  />
                  <DerivedCard
                    label="RATA-RATA FEE/TRX"
                    value={formatRp(Math.round(avgFeePerTrx))}
                    sub={monthStats ? 'dari data bulan ini' : 'estimasi historis'}
                  />
                </div>
              </div>
            )}
              ...
              <div style={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
                ...
                {projectedFee && (
                  <div style={{
                    ...
                    backgroundColor: projectedFee >= monthlyFee ? '#f0fdf4' : '#fffbeb',
                    border: `1px solid ${projectedFee >= monthlyFee ? '#bbf7d0' : '#fde68a'}`,
                  }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '4px' }}>
                      {projectedFee >= monthlyFee
                        ? `Surplus ${formatRp(projectedFee - monthlyFee)} di atas target`
                        : `Kurang ${formatRp(monthlyFee - projectedFee)} dari target — perlu akselerasi`}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <button
                onClick={handleSave}
                disabled={saving || !monthlyFee}
                style={{
                  flex: 1, padding: '13px', borderRadius: '8px', border: 'none',
                  backgroundColor: saving || !monthlyFee ? '#e5e7eb' : '#0344D8',
                  color: saving || !monthlyFee ? '#9ca3af' : '#fff',
                  fontSize: '14px', fontWeight: '700',
                  cursor: saving || !monthlyFee ? 'not-allowed' : 'pointer',
                  transition: 'all 0.2s',
                }}
              >
